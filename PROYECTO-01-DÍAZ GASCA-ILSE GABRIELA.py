...
while True:
    ...
    for categoria in categorias:
        ...
        print("Productos más buscados (# de búsquedas, producto):",busquedas[:10])
        print("Productos menos buscados (# de búsquedas, producto):",busquedas[-10:],"\n")
    ...
    #Reseña promedio por producto, anual
    for producto in lifestore_sales:
            producto_vendido=producto[1]
            if producto_vendido not in score:
                sum=0
    ...
    average_score_producto=list()
    for producto,score in producto_score.items():
        average_score_producto.append((producto,score))
    average_score_producto=sorted(average_score_producto,reverse=False)
    print("Reseña por producto (producto, score promedio):",average_score_producto,"\n")
    ...
    # Se realiza el mismo proceso que el anterior pero se itera sobre la lista de categorías en vez de la lista meses
    print("-INGRESO POR CATEGORÍA, 2020: ","\n")
    for categoria in categorias:
        productos_por_categoria_vendido=list()
        ingreso=dict()
        sum=0
        for producto in lifestore_products:
            for sale in lifestore_sales:
                ...
                if sale[1]==producto[0]:
                    if categoria==producto[3]:
                        productos_por_categoria_vendido.append(sale[1])
                        for producto_vendido in productos_por_categoria_vendido:
                            precio=producto[2]
                            ingreso[sale[1]]=precio*(productos_por_categoria_vendido.count(sale[1]))
